...
//  char		s[512];		/* Message string */
#define SIZE_OF_BUFFER 512	
	char *s = malloc(SIZE_OF_BUFFER*sizeof(char));
	if (s == NULL){
		return ;
	}
	memset(s, 0x00, sizeof(char)*SIZE_OF_BUFFER);
  ...
  */
  ...
  vsnprintf(s, SIZE_OF_BUFFER*sizeof(char), format, ap);
  ...
  */
  ...
  if (global->error_cb)
	...
	printf("mxml-private.c error:mxml: %s\n", s);
...
static pthread_key_t	_mxml_key = 0xffff;	/* Thread local storage key */
