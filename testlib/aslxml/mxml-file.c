...
#if defined (__ZOS__hwhub__)
#define MXML_TMPBUFF_SIZE	1024
#elif defined (__ZOS__beanpod__) || defined (__ZOS__watch__) || defined (__ZOS__qsee__) || defined (__ZOS__hisi__) || defined (__ZOS__tkcore__)
#define MXML_TMPBUFF_SIZE	2048
#else
#define MXML_TMPBUFF_SIZE	8192
#endif

#if defined (__ZOS__beanpod__) || defined (__ZOS__qsee__) || defined (__ZOS__hisi__) || defined (__ZOS__tkcore__) || defined (__ZOS__hwhub__)
#undef isalnum
#define isalnum silead_isalnum 
static int silead_isalnum(char ch)  
{
    return ((ch<='9'&&ch>='0')  
            ||(ch<='z'&&ch>='a')  
            ||(ch<='Z'&&ch>='A'));   
...
typedef struct _mxml_fdbuf_s		/**** File descriptor buffer ****/
{
  ...
  unsigned char	*current,		/* Current position in buffer */
		*end,			/* End of buffer */
		buffer[MXML_TMPBUFF_SIZE];		/* Character buffer */
} _mxml_fdbuf_t;
...
*/
void
mxmlSetLoneWhitespaceOff(
	int		isOff
	)
{
	_mxml_global_t *global = _mxml_global();

	global->lone_ws_node_off = isOff;
}
...
{
	_mxml_fdbuf_t	*buf = malloc(sizeof(_mxml_fdbuf_t));			/* File descriptor buffer */
	if(buf == NULL){
		return NULL;
	}
	memset(buf, 0x00, sizeof(_mxml_fdbuf_t));
	...
	*/
	 ...
	 pnode = mxml_load_data(top, buf, cb, mxml_fd_getc, MXML_NO_CALLBACK, NULL);
    ...
    mxml_save_cb_t cb)			/* I - Whitespace callback or MXML_NO_CALLBACK */
{
	...
	char	buffer[MXML_TMPBUFF_SIZE];			/* Temporary buffer */
              ...
              void           *sax_data)	/* I - SAX user data */
{
	...
	*/
	 ...
	 pnode = mxml_load_data(top, buf, cb, mxml_fd_getc, sax_cb, sax_data);
	free(buf);
	return pnode;
}
		...
		* Start of open/close tag...
			...
			* Handle open tag...
				...
				* Descend into this node, setting the value type as needed...
					*/
					//wz: if no xml header, input parent is null, then all non-element node will be discarded. is this OK? 
					//if not, need following code on.
					if(!parent)
					{
						//type = MXML_TEXT;
					}
	...
	_mxml_global_t  *global)/* I - Global data */
{
	...
	*/
	...
	case MXML_INTEGER :
		...
		snprintf(s_tmp,255,"%d", node->value.integer);
		if (mxml_write_string(s_tmp, p, putc_cb) < 0)
			return (-1);
	...
	case MXML_REAL :
		...
		snprintf(s_tmp,255,"%f", node->value.real);
		if (mxml_write_string(s_tmp, p, putc_cb) < 0)
			return (-1);
