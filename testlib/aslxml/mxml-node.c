...
{
  char	*element = (char *)malloc(1024 * sizeof(char));			/* Element text */
  if(element == NULL){
	return NULL;
  }
  memset(element, 0x00, 1024*sizeof(char));
  mxml_node_t *pnode = NULL;

  snprintf(element, sizeof(char) * 1024, "?xml version=\"%s\" encoding=\"utf-8\"?",
           version ? version : "1.0");

  pnode = mxmlNewElement(NULL, element);
  free(element);
  return pnode;
}
